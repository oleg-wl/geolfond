...
class ReestrRequest(object):
    ...
    def __init__(self, filt: str):
        ...
        self.json_data['RawOlapSettings']['measureGroup']['filters'][0][0]['selectedFilterValues'] = [filt]
    ...
    def get_record_count(self):
        ...
        response = self.session.post(self.url, headers=self.headers, json=self.json_data).json()
    ...
    def get_data_from_reestr(self):
        ...
        self.nested_data = {k:v for n, v in enumerate(response['result']['data']['values']) for k in response['result']['data']['cols'][n]}
...
class ReestrDatabase(ReestrRequest):
    ...
    def __init__(self, filt:str):
        super().__init__(filt)
        self.filt = filt
        name = [k for k,v in queries.lfilt.items() if self.filt == v] #извлечение ключа словаря фильтра для названия таблицы
