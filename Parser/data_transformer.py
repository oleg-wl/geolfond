...
class DataTransformer:
    ...
    def kdemp(self) -> str:
        ...
        d = {'Котировка':['Бензин-регуляр92', 'Дизель'], 'Средняя цена с начала месяца':[self.abdt.iloc[0,0], self.abdt.iloc[0,1]], 'Норматив по НК РФ':norm, 'Лимит отклонения цены':[0.1, 0.2]}
        self.logger.info(f'Средняя цена АБ92-регуляр {d["Средняя цена с начала месяца"][0]} \nСредняя цена ДТ {d["Средняя цена с начала месяца"][1]}')

        dx = pd.DataFrame(d)
        dx['Верхняя граница лимита'] = dx['Норматив по НК РФ'] * dx['Лимит отклонения цены'] + dx['Норматив по НК РФ']
        dx['Отклонение от норматива, %'] = ((dx['Средняя цена с начала месяца'] - dx['Норматив по НК РФ'])/dx['Норматив по НК РФ'] * 100).round(2)
        dx['Отклонение от верхней границы цены'] = dx['Средняя цена с начала месяца'] - dx['Верхняя граница лимита'] 
