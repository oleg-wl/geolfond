...
def construct_df(nested_data: dict):
    ...
    cols = [
            ...
            'Сведения о пользователе недр','INN','Наименоваение недропользователя','prev_owner',
            'Last','Year',
            'Координаты','N', 'E',
            'Сведения о переоформлении лицензии на пользование недрами','Ранее выданные лицензии',
            'prev_lic', 'prev_date',
            'forw_lic', 'forw_date',
            ]
    types = {
            ...
            'Last':'bool',
            'N':'str', 'E':'str',
            'prev_lic':'str', 'prev_date':'datetime64[D]',
            'forw_lic':'str', 'forw_date':'datetime64[D]',
            'Наименование участка недр':'str', 'Вид полезного ископаемого':'str',
            'СубъектРФ':'str', 'INN':'str', 'Year':'int',
            }
    

    df = pd.DataFrame(nested_data).rename(columns={
                'Дата':'Дата лицензии',
    ...
    df['Year'] = pd.to_datetime(df['Дата лицензии'], format='%Y-%m-%d', yearfirst=True).dt.year #Год лицензии
    df['Last'] = np.where(df['Сведения о переоформлении лицензии на пользование недрами'].isna(), True, False) #Условный столбец последняя лицензия
    ...
    df['prev_date'] = pd.to_datetime(df['prev_date'], format='%d.%m.%Y', errors='ignore', dayfirst=True)
    ...
    df['forw_date'] = pd.to_datetime(df['forw_date'], format='%d.%m.%Y', errors='ignore', dayfirst=True)
    ...
    coords = df['Координаты'].str.extractall(pattern_coord, re.MULTILINE).sort_values(by='N', ascending=False).droplevel(1)
    ...
    df = pd.concat([df,prev_owner_df], axis=1, join='outer').rename_axis('Государственный регистрационный номер')
    ...
    for i in range(100):
        ...
        df.loc[df.index.isin(lst2),['Наименование участка недр']] = names_df.loc[names_df.index.isin(lst2)]

    df = (df[cols].astype(dtype=types)
          .where(~df.isnull(), '') #заменить значения NaN на пустые для выгрузки в эксель
          )
    return df
...
def save_in_excel(path: str, dataframe_to_save:pd.DataFrame, name_for_sheet: str):
    ...
    if os.path.exists(path):
        with pd.ExcelWriter(path, mode='a', if_sheet_exists='replace') as writer:
            dataframe_to_save.to_excel(writer, sheet_name=name_for_sheet, merge_cells=False, freeze_panes=(1,0), na_rep='')

    else:
        dataframe_to_save.to_excel(path, sheet_name=name_for_sheet, merge_cells=False, freeze_panes=(1,0), na_rep='') 