...
def file_list(dir_path):
    ...
    dir = pathlib.Path(
        os.path.normpath(os.path.expanduser(dir_path))
    )  # Полный путь с учетом юзера в dir_path
    file_list = []
...
def new_licenses(file_list, dir_path):
    ...
    result_df.to_excel(
        f"{os.path.abspath(os.path.expanduser(dir_path))}\compare_result_{now}.xlsx",
        sheet_name="CompairedData",
    )

    # result_df = result_df.where(result_df['Сведения о пользователе недр_old'] != result_df['Сведения о пользователе недр_new']).drop()
    print(
        f"Я сравнил {re.findall(file_pattern, dir_1)} и {re.findall(file_pattern,dir_2)}. Результат сохранил в {dir_path}"
    )
    return result_df
